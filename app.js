...
function generateRandomSurah(){
   return  quranVerses.surahs.references[getNumberBetween(0, 113)];
}

function doQuiz(){
    const buttonIds = shuffle([1,2,3,4])
    document.querySelector(`#button${buttonIds[0]}`).innerHTML = ayah?.surah?.name;
    document.querySelector(`#button${buttonIds[1]}`).innerHTML = generateRandomSurah().name;
    document.querySelector(`#button${buttonIds[2]}`).innerHTML = generateRandomSurah().name;
    document.querySelector(`#button${buttonIds[3]}`).innerHTML = generateRandomSurah().name;
    document.querySelector(`#button${buttonIds[0]}`).onclick = function(){
        alert("الإجابة صحيحة، ما شاء الله")
        displayAyah();
    }
    document.querySelector(`#button${buttonIds[1]}`).onclick = function(){
        alert("الإجابة خاطئة، حاول مرة أخرى")
    }
    document.querySelector(`#button${buttonIds[2]}`).onclick = function(){
        alert("الإجابة خاطئة، حاول مرة أخرى")
    }
    document.querySelector(`#button${buttonIds[3]}`).onclick = function(){
        alert("الإجابة خاطئة، حاول مرة أخرى")
    }
